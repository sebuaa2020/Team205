...
#include <stdlib.h>
#include <map>
#include "webSocket/debug_log.h"
#include "webSocket/network_interface.h"
...
void Network_Interface::ctl_event(int fd, bool flag){
	...
	if(flag){
		...
		if(fd != listenfd_)
			DEBUG_LOG("客户端连接成功，客户端句柄号为fd: %d", fd);
	}
	else{
		close(fd);
		delete websocket_handler_map_[fd];
		websocket_handler_map_.erase(fd);
		DEBUG_LOG("句柄为fd: %d的客户端已离开或切换了网页", fd);
	}
}
