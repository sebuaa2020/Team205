...
#include <sstream>
#include "../include/team205_robot/qnode.hpp"
#include <QDebug>
/*****************************************************************************
** Namespaces
*****************************************************************************/
...
QNode::QNode(int argc, char** argv ) :
    ...
    {
//    读取topic的设置
    QSettings topic_setting("topic_setting","team205_robot");
    ...
    power_min=topic_setting.value("power_min","23.1").toString();
    power_max=topic_setting.value("power_max","29.4").toString();
...
bool QNode::init() {
	ros::init(init_argc,init_argv,"team205_robot");
	if ( ! ros::master::check() ) {
		return false;
	}
    ...
    pos_sub=n.subscribe(pose_topic.toStdString(),1000,&QNode::poseCallback,this);
     ...
     //速度控制话题
     cmd_pub = n.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/teleop", 1000);
	start();
	return true;
}
...
bool QNode::init(const std::string &master_url, const std::string &host_url) {
	...
	ros::init(remappings,"team205_robot");
	if ( ! ros::master::check() ) {
		return false;
	}
	ros::start(); // explicitly needed since our nodehandle is going out of scope.
