#include "../include/team205_robot/qrviz.hpp"
...
void QRviz::Display_LaserScan(bool enable,QString topic)
{
    if(laser_==NULL)
    {
        ...
        laser_->subProp("Style")->setValue("Squares");
        laser_->subProp("Size (m)")->setValue("0.03");
        laser_->subProp("Alpha")->setValue("1");
        laser_->subProp("Color Transformer")->setValue("Intensity");
        laser_->subProp("Queue Size")->setValue("10");
        laser_->subProp("Autocompute Intensity Bounds")->setValue(true);
    }
    else{
        delete laser_;
        laser_=manager_->createDisplay("rviz/LaserScan","QLaser",enable);
...
void QRviz::Display_PointCloud2(bool enable,QString topic)
{
    if(point_==NULL)
    {
        point_=manager_->createDisplay("rviz/PointCloud2","QPoint",enable);
        ROS_ASSERT(point_);
        point_->subProp("Topic")->setValue(topic);
        point_->subProp("Style")->setValue("Flat Squares");
        point_->subProp("Size (m)")->setValue("0.02");
        point_->subProp("Alpha")->setValue("1");
        point_->subProp("Decay Time")->setValue("0");
        point_->subProp("Color Transformer")->setValue("RGB8");
        point_->subProp("Queue Size")->setValue("10");
    }
    else{
        delete point_;
        point_=manager_->createDisplay("rviz/PointCloud2","QPoint",enable);
    ...
    qDebug()<<"topic:"<<topic;
    point_->setEnabled(enable);
    manager_->startUpdate();
}
//显示
void QRviz::Display_Marker(bool enable,QString topic)
{
    if(marker_==NULL)
    {
        marker_=manager_->createDisplay("rviz/Marker","QMarker",enable);
        ROS_ASSERT(marker_);
        marker_->subProp("Marker Topic")->setValue(topic);
...
// "rviz/SetGoal";
 ...
 void QRviz::Add_Waypoint()
 {
     ...
     //设置当前frame
     //manager_->setFixedFrame("map");
     //设置当前使用的工具为AddWaypoint（实现在地图上标点）
     tool_manager_->setCurrentTool( current_tool );
