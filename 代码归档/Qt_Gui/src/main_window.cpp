...
#include <iostream>
#include "../include/team205_robot/main_window.hpp"
...
void MainWindow::initVideos()
{
   ...
   QSettings video_topic_setting("video_topic","team205_robot");
...
void MainWindow::connections()
{
   ...
   connect(ui.pushButton_k,SIGNAL(clicked()),this,SLOT(slot_cmd_control()));
   ...
   connect(ui.add_point_btn,SIGNAL(clicked()),this,SLOT(slot_add_2D_Point()));
   connect(ui.goto_point_btn,SIGNAL(clicked()),this,SLOT(slot_goto_2D_Point()));
   //设置返航点
...
void MainWindow::slot_set_return_point()
{
    ...
    //写入setting
    QSettings settings("return-position", "team205_robot");
...
void MainWindow::slot_add_2D_Point()
{
  map_rviz->Add_Waypoint();
}
void MainWindow::slot_goto_2D_Point()
{
    QString bash="navigation";
    ui.base_cmd_output->append(bash);
    base_cmd->write(bash.toLocal8Bit()+'\n');
  bool bOk = false;
  QString point_Name = QInputDialog::getText(this,
                                         "goto Waypoint",
                                         "请输入巡航点",
                                         QLineEdit::Normal,
                                         "1",
  ...
  if (point_Name=="10") 
  {
      QMessageBox::information(NULL, "goto Waypoint", "No such a point", 
                         QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes); 
  }                                      
  if (bOk && !point_Name.isEmpty()) {
      ui.base_cmd_output->append(point_Name);
      base_cmd->write(point_Name.toLocal8Bit()+'\n');
  }
}
...
//        qDebug()<<topic_box->currentText()<<alpha->text()<<scheme->currentText();
    ...
    else if(dis_name=="Marker")
    {
        QComboBox *topic_box=(QComboBox *) ui.treeWidget_rviz->itemWidget(parentItem->child(1),1);
        map_rviz->Display_Marker(enable,topic_box->currentText());
    }
...
void MainWindow::slot_choose_topic(QTreeWidgetItem *choose)
{
    ...
    else if(choose->text(0)=="PointCloud2")
    {
        ...
        QComboBox *Point_Topic=new QComboBox;
        Point_Topic->setMaximumWidth(150);
        Point_Topic->addItem("/camera/depth/points");
        Point_Topic->setEditable(true);
        widget_to_parentItem_map[Point_Topic]=choose;
        ui.treeWidget_rviz->setItemWidget(choose->child(1),1,Point_Topic);
        //绑定值改变了的事件
        connect(Point_Topic,SIGNAL(currentTextChanged(QString)),this,SLOT(slot_treewidget_item_value_change(QString)));

    }
    else if(choose->text(0)=="Marker")
    {

        QComboBox *Marker_Topic=new QComboBox;
        Marker_Topic->setMaximumWidth(150);
        Marker_Topic->addItem("/waypoints_marker");
        Marker_Topic->setEditable(true);
        widget_to_parentItem_map[Marker_Topic]=choose;
        ui.treeWidget_rviz->setItemWidget(choose->child(1),1,Marker_Topic);
        //绑定值改变了的事件
        connect(Marker_Topic,SIGNAL(currentTextChanged(QString)),this,SLOT(slot_treewidget_item_value_change(QString)));
...
void MainWindow::slot_cmd_control()
{
    ...
    switch (key) {
        ...
        break;
        case 'k':
            qnode.move_base(is_all?'K':'k',liner,turn);
        break;
        case 'l':
...
void MainWindow::quick_cmds_check_change(int state)
{
    ...
    if(is_checked)
    {
        ...
        quick_cmd->waitForStarted();
        qDebug()<<bash;
        quick_cmd->write(bash.toLocal8Bit()+'\n');
        connect(quick_cmd,SIGNAL(readyReadStandardOutput()),this,SLOT(quick_cmd_output()));
         connect(quick_cmd,SIGNAL(readyReadStandardError()),this,SLOT(quick_cmd_error_output()));
    }
    else{
...
void MainWindow::on_build_map_btn_clicked(bool check ) 
{
    bool bOk = false;
  QString button = QInputDialog::getText(this,
                                         "build Map",
                                         "是否建图",
                                         QLineEdit::Normal,
                                         "N",
  ...
  if (bOk && !button.isEmpty()) {
      ui.base_cmd_output->append(button);
      base_cmd->write(button.toLocal8Bit()+'\n');
  }
}
void MainWindow::on_save_map_btn_clicked(bool check ) 
{
    QString bash="save";
    ui.base_cmd_output->append(bash);
    base_cmd->write(bash.toLocal8Bit()+'\n');
    //Map saved successfully
    QMessageBox::information(NULL, "build Map", "Map saved successfully", 
                         QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
    //QString bash="killall gzserver ";
    //ui.base_cmd_output->append(bash);
    //base_cmd->write(bash.toLocal8Bit()+'\n');
}
void MainWindow::on_obj_detect_btn_clicked(bool check )
{
    QString bash="detect";
    ui.base_cmd_output->append(bash);
    base_cmd->write(bash.toLocal8Bit()+'\n');
}
void MainWindow::on_add_btn_clicked(bool check )
{
    QString bash="add";
    ui.base_cmd_output->append(bash);
    base_cmd->write(bash.toLocal8Bit()+'\n');
...
void MainWindow::on_terminal_connect_clicked(bool check ) 
{
    base_cmd=new QProcess; 
    base_cmd->start("bash");
    base_cmd->waitForStarted();
    ui.base_cmd_output->append("New terminal success!");
    QString bash="rosrun robot_sim_demo robot";
    ui.base_cmd_output->append(bash);
    base_cmd->write(bash.toLocal8Bit()+'\n');
    ui.terminal_connect->setEnabled(false);
    ui.line_edit_cmd->setClearButtonEnabled(true);
    connect(base_cmd,SIGNAL(readyReadStandardOutput()),this,SLOT(base_cmd_output()));
     connect(base_cmd,SIGNAL(readyReadStandardError()),this,SLOT(base_cmd_error_output()));
}
void MainWindow::on_terminal_disconnect_clicked(bool check )
{
    base_cmd->close();
    base_cmd->waitForFinished();
    //base_cmd->kill();
    ui.base_cmd_output->append("Kill terminal success!");
    ui.terminal_connect->setEnabled(true);
}
void MainWindow::on_send_cmd_clicked(bool check ) 
{
    QString bash=ui.line_edit_cmd->text();
...
//执行一些命令的回显
void MainWindow::quick_cmd_output()
{

    ui.quick_cmd_output->append(quick_cmd->readAllStandardOutput().data());
    //ui->quick_cmd_output->moveCursor(QTextCursor::End);

}
//执行一些命令的错误回显
void MainWindow::quick_cmd_error_output()
{
    ui.quick_cmd_output->append("<font color=\"#FF0000\">"+quick_cmd->readAllStandardError()+"</font> ");
    //ui->quick_cmd_output->moveCursor(QTextCursor::End);
    //QMessageBox::information(NULL,"Error",quick_cmd->readAllStandardError().data());
}
//执行一些命令的回显
void MainWindow::base_cmd_output()
{

    ui.base_cmd_output->append(base_cmd->readAllStandardOutput().data());
    //ui->base_cmd_output->moveCursor(QTextCursor::End);
}
//执行一些命令的错误回显
void MainWindow::base_cmd_error_output()
{
    ui.base_cmd_output->append(base_cmd->readAllStandardError().data());
    //ui.base_cmd_output->append("<font color=\"#FF0000\">"+base_cmd->readAllStandardError()+"</font> ");
    
    
    //QMessageBox::information(NULL,"Error",base_cmd->readAllStandardError().data());
}
//析构函数
...
void MainWindow::ReadSettings() {
    QSettings settings("Qt-Ros Package", "team205_robot");
    ...
    QSettings return_pos("return-position","team205_robot");
    ...
    //读取快捷指令的setting
    QSettings quick_setting("quick_setting","team205_robot");
